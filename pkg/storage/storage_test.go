...
FROM resources r  
LEFT JOIN metadata m ON r.id = m.resource_id WHERE kind = $1 AND name LIKE $2 
GROUP BY r.id 
LIMIT 200 OFFSET 0`
...
LIMIT 200 OFFSET 0`, query)
    ...
    json_object_agg(m.key, m.value) FILTER (WHERE m.key IS NOT NULL) AS meta_data 
FROM resources r  
LEFT JOIN metadata m ON r.id = m.resource_id WHERE kind = $1 
GROUP BY r.id 
LIMIT 200 OFFSET 0`, query)
    ...
    json_object_agg(m.key, m.value) FILTER (WHERE m.key IS NOT NULL) AS meta_data 
FROM resources r  
LEFT JOIN metadata m ON r.id = m.resource_id WHERE kind = $1 AND name = $2 AND uuid != $3 AND external_id like $4 
GROUP BY r.id 
LIMIT 200 OFFSET 0`, query)
